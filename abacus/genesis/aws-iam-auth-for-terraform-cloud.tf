    ...
    "Statement" : [
      {
        ...
        "Condition" : {
          ...
          "StringLike" : {
            "${local.terraform_cloud_hostname}:sub" : "organization:${local.terraform_cloud_organization_name}:project:${local.terraform_cloud_project_name}:workspace:${local.terraform_cloud_workspace_name}:run_phase:*"
          }
        }
      }
    ]
